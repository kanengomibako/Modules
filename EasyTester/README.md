...
- 導通チェック設定モード
  ...
  ※ ブザー音の周波数は目安です。圧電スピーカーの種類によっては、大幅に違う周波数の音が聞こえる場合があります。
...
- ADCキャリブレーションモード
  ...
  アナログ・デジタルコンバーター（ADC）の誤差を補正します。＜0V＞ではCOM端子とV・Ω端子をショートさせ、補正のオフセット値を算出します。もう一方では任意の電圧（1～20V）の基準電圧源を設定・接続し、補正の傾きを算出します。
...
- 抵抗キャリブレーションモード
  ...
  抵抗測定の誤差を補正します。基準抵抗を順番に接続し、補正の傾きとオフセット値を算出します（10Ω抵抗を接続し実行、200Ω抵抗を接続し実行……と繰り返す）。カーソルは自動で移動するので、やり直す場合は電源を入れ直してください。
